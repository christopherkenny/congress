...
#' @param number Treaty assigned number. Numeric.
#' @param suffix Treaty partition letter value. Character.
#' @param item Information to request. Can be `'actions'`, or `'committees'`.
...
cong_treaty <- function(congress = NULL, number = NULL, suffix = NULL, item = NULL,
  ...
  endpt <- treaty_endpoint(congress = congress, number = number, suffix = suffix, item = item)
...
treaty_items <- c('actions', 'committees')
